...
const MenuManagement = () => {
  ...
  const menuItems = [
    { 
      ...
      href: "/pengaduan/permohonan-informasi",
      icon: <FileText size={15} />,
      desc: "Manajemen berkas masuk"
    },
    { 
      name: "Pengajuan Keberatan", 
      href: "/pengaduan/pengajuan-keberatan",
      icon: <ShieldCheck size={15} />,
      desc: "Layanan sengketa informasi"
    },
  ];

  return (
    <div className="px-2 py-4">
      {/* Label Kategori - Opsional tapi bikin pro */}
        ...
        >
          <div className="flex items-center gap-3">
            <div className={`p-2.5 rounded-2xl transition-all duration-500 ${
              isOpen 
                ? "bg-red-600 text-white shadow-lg shadow-red-200 rotate-0" 
                : "bg-slate-100 text-slate-400 group-hover:bg-red-50 group-hover:text-red-500"
            }`}>
              <Layers size={18} />
            </div>
            <div className="text-left">
              <span className="text-sm font-black tracking-tight block">Data Master</span>
              {!isOpen && <span className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">Database PPID</span>}
            </div>
          </div>
            ...
            {menuItems.map((item, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && (
                    <div className="absolute right-3 w-1.5 h-1.5 rounded-full bg-red-500 shadow-[0_0_10px_#ef4444]"></div>
                  )}
                </Link>
              );
            })}
