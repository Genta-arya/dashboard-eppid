...
const Dashboard = () => {
  ...
  const totalInformasi =
    analytics?.jenis_layanan?.find((l) => l.type === "PERMINTAAN_INFORMASI")
      ?._count || 0;
  const totalKeberatan =
    analytics?.jenis_layanan?.find((l) => l.type === "KEBERATAN")?._count || 0;
  const customMarkerIcon = L.divIcon({
    html: renderToString(
      <div className="text-red-600 bg-white p-1 rounded-full shadow-lg border-2 border-red-500 flex items-center justify-center">
        <MapPin size={20} fill="currentColor" className="text-red-600" />
      </div>,
    ),
    className: "custom-div-icon", // Menghapus styling default kotak putih leaflet
    iconSize: [30, 30],
    iconAnchor: [15, 30], // Agar ujung bawah pin pas di koordinat
  });
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-12">
      <Helmet>
        <title>Dashboard Intelligence | e-PPID</title>
      </Helmet>
      <Headers user={user} />
        ...
        {loading ? (
          ...
          <div className="pt-4 md:pt-8 space-y-6 md:space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
              <div>
                <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                  ...
                  <Sparkles className="text-red-500 w-5 h-5 md:w-6 md:h-6 animate-pulse" />
                </h1>
                <p className="text-slate-500 font-medium mt-1 text-sm italic">
                  Sistem Monitoring Pelayanan • Tahun {year}
                </p>
              </div>
              ...
              <div className="flex w-full md:w-auto md:justify-end  items-center bg-white p-1.5 rounded-2xl shadow-sm border border-slate-200">
                ...
                <button
                  onClick={fetchData}
                  className="bg-red-600 hover:bg-red-700 text-white p-2.5 rounded-xl transition-all shadow-lg shadow-red-200 active:scale-95"
                >
                  <Search size={18} />
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6">
              ...
              <StatCard
                label="Rerata Respon"
                value={analytics?.average_response_time || "0h"}
                isCountUp={false}
                icon={<Clock size={20} />}
                color="orange"
              />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
                ...
                <div className="bg-white p-5 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm relative overflow-hidden group">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
                    <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                      <TrendingUp className="text-blue-500 w-5 h-5" /> Tren
                      Pelayanan & Trafik
                    </h3>
                  ...
                  <div className="h-[350px] w-full">
                    <Line
                      data={{
                        labels:
                          analytics?.traffic_pengunjung?.map((v) => v.bulan) ||
                          [],
                        datasets: [
                          {
                            label: "Visitor",
                            data:
                              analytics?.traffic_pengunjung?.map(
                                (v) => v.total,
                              ) || [],
                            borderColor: "#3b82f6",
                          ...
                          },
                          {
                            ...
                            data:
                              analytics?.traffic_ticket?.map((v) => v.total) ||
                              [],
                            borderColor: "#ef4444",
                ...
                <div className="bg-slate-950 rounded-[2rem] p-8 text-white shadow-2xl relative overflow-hidden group">
                  ...
                  <div className="relative z-10 space-y-6">
                    ...
                    {aiLoading ? (
                      ...
                      <p className="text-sm leading-relaxed text-slate-300 italic font-medium line-clamp-2">
                        "
                        {aiInsight?.insight ||
                    ...
                    <button
                      ...
                      className="px-6 py-3 bg-red-600 hover:bg-red-500 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 transition-all active:scale-95"
                    >
              ...
              <div className="lg:col-span-4 space-y-6 md:space-y-8">
                ...
                <div className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm space-y-6">
                  <h3 className="font-bold text-slate-800 flex items-center gap-2 px-2 text-md">
                    <MapPin size={18} className="text-red-500" /> Lokasi Akses
                  </h3>
                  <div className="h-60 rounded-3xl overflow-hidden border border-slate-50 relative z-0 shadow-inner">
                    ...
                    >
                      ...
                      {analytics?.visitor_locations?.map((loc, i) => (
                        ...
                        >
                          <Popup>
                            <span className="font-bold">IP: {loc.ip}</span>
                          </Popup>
                        </Marker>
                      ))}
                    </MapContainer>
                    ...
                    <div className="bg-slate-900 p-5 rounded-3xl text-white space-y-4 animate-in slide-in-from-top-4 relative border border-white/5 shadow-2xl">
                      ...
                      <div className="flex gap-3 items-start">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] leading-relaxed break-words whitespace-normal font-medium text-slate-200">
                      ...
                      <div className="pt-3 border-t border-white/10 flex justify-between gap-4">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs font-mono text-emerald-400 font-black tracking-tighter">
                            {selectedLocation.ip}
                          </p>
                        </div>
                        <div className="min-w-0 text-left">
                          ...
                          <div className="flex items-center gap-1 justify-end">
                            ...
                            <p className="text-[10px] text-slate-300  w-24">
                ...
                <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
                  <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2 text-md">
                    <Clock size={18} className="text-indigo-500" /> Matriks
                    Respons
                  </h3>
                  <div className="space-y-4">
                    {analytics?.response_time_detail?.map((item, i) => {
                      const isInformasi =
                        item.layanan === "PERMINTAAN_INFORMASI";
                      return (
                        <div
                          key={i}
                          className={`flex justify-between items-center p-4 rounded-2xl border-l-4 transition-all group ${isInformasi ? "bg-indigo-50/50 border-indigo-500" : "bg-rose-50/50 border-rose-500"}`}
                        >
                          <div className="min-w-0">
                            <p
                              className={`text-[9px] font-black uppercase tracking-widest mb-1 ${isInformasi ? "text-indigo-400" : "text-rose-400"}`}
                            >
                              Tipe Layanan
                            </p>
                          ...
                          <span
                            className={`text-sm font-black italic shrink-0 ml-4 ${isInformasi ? "text-indigo-600" : "text-rose-600"}`}
                          >
                            {item.avg_response}
                          </span>
                        </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="p-5 bg-blue-50 rounded-2xl border border-blue-100">
                  <h4 className="font-bold text-blue-800 mb-2 flex items-center gap-2">
                    <Users size={16} /> Tren Pengunjung
                  </h4>
                  <p className="text-xs text-blue-700 leading-relaxed">
                    {aiInsight.trend_pengunjung}
                  </p>
                </div>
                <div className="p-5 bg-purple-50 rounded-2xl border border-purple-100">
                  <h4 className="font-bold text-purple-800 mb-2 flex items-center gap-2">
                    <Ticket size={16} /> Tren Layanan
                  </h4>
                  <p className="text-xs text-purple-700 leading-relaxed">
                    {aiInsight.trend_layanan}
                  </p>
                </div>
              </div>

              <section className="bg-slate-900 p-6 rounded-3xl text-white">
                <h4 className="text-xs font-black text-red-400 uppercase tracking-[0.2em] mb-4">
                ...
                <ul className="space-y-4">
                  {aiInsight.rekomendasi?.map((item, idx) => (
                    ...
                    >
                      <span className="shrink-0 w-5 h-5 bg-red-500/20 text-red-400 rounded-full flex items-center justify-center font-bold text-[10px] border border-red-500/30">
                        {idx + 1}
                      </span>
                      {item}
                    </li>
  ...
  return (
    <div className="bg-white p-5 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col justify-between hover:shadow-xl transition-all duration-300 group">
      <div className="flex justify-between items-start mb-4">
        <div
          className={`p-3 rounded-xl ${themes[color]} border transition-all duration-500 group-hover:scale-110 shadow-sm`}
        >
          {icon}
        </div>
      </div>
